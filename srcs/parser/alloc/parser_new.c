...
static int	s_build_token_expand(int mode, t_parser *parser)
{
	int		i;

	i = 0;
	if (mode == F_PARSING_GLOB_BRACE)
	{
	...
	{
		parser->token_list[i++] = &g_token_var_inhibitor_dquote;
		parser->token_list[i++] = &g_token_var_inhibitor_quote;
	}
...
static int	s_parser_new_part2(t_parser **parser, int mode)
{
	...
	else if (mode == F_PARSING_VAR)
		(*parser)->unstack_func = &parser_build_list_unstack_lexer_var;
...
int			parser_new(t_parser **parser, const char *in, t_sh *sh, int mode)
{
	...
	if (lexer_tokens_alloc((*parser)->lexer) != ST_OK)
		return (ST_MALLOC);
	if (mode == F_PARSING_VAR || mode == F_PARSING_GLOB_BRACE)
		s_build_token_expand(mode, *parser);
	else
