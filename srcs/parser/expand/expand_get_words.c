...
static int	s_get_new_length(char *str)
{
	int		i;
	int		len;
...
static char	*s_build_str(char *content, char *input)
{
	int		k;
	int		j;
	char	*str;
	...
	if ((str = ft_strnew(s_get_new_length(input))) == NULL)
		return (NULL);
	j = 0;
	k = 0;
	while (input[j] != '\0')
	{
		if (s_is_token(input[j]) == 1 && s_is_inhibited(input, input + j) == 0)
			str[k++] = '\\';
		str[k++] = input[j];
		j++;
	}
				...
				int *i)
{
								...
								|| TOKEN_TYPE(*i) == TT_NAME))
	{
		if (TOKEN_TYPE(*i) == TT_INHIBITOR)
			is_inhibited = (is_inhibited == 0) ? 1 : 0;
		if (is_inhibited == 1)
			tmp = s_build_str(*content, TOKEN_CONTENT(*i));
		else
