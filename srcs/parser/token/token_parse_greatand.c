...
static int	s_parse_right_redir_proc(t_proc *p, t_parser *parser, int *i,
				int *fd)
{
	int		ret;
	char	*str;
	...
	{
		*fd = -1;
		return (token_parse_utils_gen_token_after_dash(p, parser,
															parser->lexer, i));
	}
	token_parse_utils_skip_separators(parser->lexer, i, NULL);
	if ((ret = token_parse_utils_check_char_to_fd(P_TOKEN_CONTENT(*i),
															fd)) != ST_OK)
	{
		str = NULL;
		if ((ret = token_parse_utils_get_full_word(&str,
												parser->lexer, i)) != ST_OK)
			return (ret);
		(*i)--;
		if ((ret = token_parse_utils_open_new_fd(p, str, fd,
