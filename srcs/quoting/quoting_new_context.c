											...
											int tokenid)
{
	...
	if (!(ret = 0) && context->option == OPTION_NONE)
	{
		...
		termcaps_initialize(context->sh, s_set_prompt_quoting(tokenid),
							&child_context);
		if ((ret = s_first_loop_check(&cmd, &child_context, context))
			== ST_MALLOC)
			return (ST_MALLOC);
