...
static int	s_termcaps_identify_input(const unsigned int c,
											...
											t_internal_context *context)
{
	if (input_type == MINISHELL__INPUT_TYPE_PRINT)
		termcaps_string_to_command_line(input_buffer_size,
...
static int			s_print_first_prompt()
{
	int				ret;
	...
	else if (context.state != STATE_REGULAR)
	{
		write (shell_fd(), "\r", 2);
		ASSERT(termcaps_display_command_line(&context.command_line));
	}
...
static int			s_termcaps_read_loop(const int fd)
{
	...
	while (context.state != STATE_EXIT && context.state != STATE_CONTINUE)
	{
		...
		s_termcaps_identify_input(input_buffer[0], &input_type, &input_size_missing);
		if (input_buffer_size + input_size_missing > sizeof(input_buffer))
			FATAL("Buffer overflow on input_buffer, need %zu but have %zu",
			input_buffer_size + input_size_missing, sizeof(input_buffer));
		...
		s_termcaps_treat_input(input_type, input_buffer_size, input_buffer, &context);
		if (context.state == STATE_REGULAR)
		{
			ASSERT(termcaps_display_command_line(&context.command_line));
