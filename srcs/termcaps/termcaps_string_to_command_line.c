										  ...
										  t_list_head *command_line)
{
	...
	{
		character_bytes_count = termcaps_get_character_bytes_count(input_buffer_size - input_buffer_index,
														  input_buffer + input_buffer_index,
														  &character_missing_bytes_count);
		if (character_bytes_count == 0 || character_missing_bytes_count > 0)
			FATAL("get_character_bytes_count() failed character bytes_count %zu missing_bytes_count %zu",
				character_bytes_count, character_missing_bytes_count);
