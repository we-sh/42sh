...
static int	s_get_word(const char *pt, int *word)
{
	...
	{
		*word = ft_atoi(pt);
		return (count_func(pt, ft_isdigit, 1));
	}
	return (0);
}
							...
							t_event *event)
{
	if (ft_isalpha(pt[1]))
	{
		event->token_size = 1 + count_func(pt + 1, ft_isalpha, 1);
		event->history_index =
			history_search_prefix(history, pt + 1, event->token_size - 1, -1);
	}
	else if (pt[1] == '?' && ft_isalpha(pt[2]))
	{
		event->token_size = 2 + count_func(pt + 2, ft_isalpha, 1);
		event->history_index =
...
static int	s_event_index(const char *pt, t_event *event)
{
	if (ft_isdigit(pt[1]))
	{
		event->token_size = 1 + count_func(pt + 1, ft_isdigit, 1);
		event->history_index = ft_atoi(pt + 1);
	}
	else if (pt[1] == '-' && ft_isdigit(pt[2]))
	{
		event->token_size = 2 + count_func(pt + 2, ft_isdigit, 1);
		event->history_index = ft_atoi(pt + 1);
	}
	else
		return (0);
						...
						t_event *event)
{
	...
	{
		event->token_size = count_func(pt, ft_isspace, 0);
		return (0);
	}
