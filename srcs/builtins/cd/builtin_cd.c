	...
	if (p->bltin_status == ST_OK)
	{
		...
		else
		{
			if (p->argc == 1)
			{
				...
				else if ((p->bltin_char = ft_strdup(str)) == NULL)
					return (ST_MALLOC);
			}
			else
			{
				if (ft_strcmp(p->argv[1], "-") == 0)
				{
			...
			if (p->bltin_char == NULL)
				return (ST_MALLOC);
			else if ((p->bltin_char = builtin_cd_remove_dotdot(p->bltin_char)) == NULL)
				return (ST_MALLOC);

			ret = builtin_cd_chk_path(&p->bltin_char, option_is_set(&p->bltin_opt_head, ST_BLTIN_CD_OPT_P2));
			p->bltin_status = ret;
		}
