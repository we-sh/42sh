...
static int	s_set_bltin_char(t_sh *sh, t_proc *p)
{
	...
	if (p->bltin_status == ST_OK && p->bltin_char == NULL)
		return (ST_MALLOC);
	return (ST_OK);
}
	...
	if (p->bltin_status != ST_OK)
	{
		...
		else
		{
			if (p->argc == 2 && ft_strcmp(p->argv[1], "-") == 0)
				display_status(p->bltin_status, "cd",
					env_get(p->envp, "OLDPWD"));
			else if (p->argc == 2)
				display_status(p->bltin_status, "cd", p->argv[1]);
			else if (p->argc == 1)
				display_status(p->bltin_status, "cd", env_get(p->envp, "HOME"));
			else
				display_status(p->bltin_status, "cd", NULL);
		}
		return (EXIT_FAILURE);
	}
	return (EXIT_SUCCESS);
...
static int	s_after(t_sh *sh, t_proc *p)
{
	...
	if (p->bltin_status == ST_OK)
	{
		...
		if ((ret = env_set(&sh->envp, "OLDPWD", sh->pwd, HTABLE_MODIF))
			!= ST_OK)
			return (ret);
		if ((ret = env_set(&sh->envp, "PWD", p->bltin_char, HTABLE_MODIF))
			!= ST_OK)
			return (ret);
