...
t_job	*s_job_is_prefixed_by(char const *prefix)
{
	t_list	*j_pos;
	t_list	*j_head;
	t_job	*j;
...
t_job	*s_job_contains(char const *str)
{
	t_list	*j_pos;
	t_list	*j_head;
	t_job	*j;
...
t_job	*s_job_with_position(int const position)
{
	size_t	j_total;
	t_list	*j_pos;
...
t_job	*job_by_name(char const *name)
{
	...
	if (ft_isdigit(name[1]))
		return (job_by_id(ft_atoi(name + 1)));
	if (name[1] == '-')
		return (s_job_with_position(-2));
	if (name[1] == '%' || name[1] == '+')
		return (s_job_with_position(-1));
	if (name[1] == '?')
		return (s_job_contains(name + 2));
	return (s_job_is_prefixed_by(name + 1));
}
